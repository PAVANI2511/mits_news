...
import { FiUsers, FiInfo, FiRefreshCw } from 'react-icons/fi';
...
const FollowingFeed = () => {
  ...
  const loadFeed = async (pageNum, reset = false) => {
    ...
    setError('');
    try {
      const res = await postsAPI.getFollowingFeed({
        page: pageNum,
        page_size: 5
      });
      ...
      setError("Failed to load following feed. Make sure you follow users who have published posts.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="space-y-6">
        <div className="bg-card border border-border p-4 rounded-2xl flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-black text-sm uppercase tracking-wider text-text">Following Feed</h2>
              <p className="text-[10px] text-gray-500 font-semibold">Articles published by users you follow</p>
            </div>
          </div>
        </div>
